...
data class ProjectInfo(
    ...
    val projectCacheDirHash: String
) {
    companion object {
        ...
        fun writeToDirectory(context: Context, directory: File, projectTreeUri: Uri, projectCacheDirHash: String) {
            val name = DocumentFile.fromTreeUri(context, projectTreeUri)?.name ?: "Unknown Project"
            val projectInfo = ProjectInfo(projectTreeUri.toString(), name, projectCacheDirHash)
        ...
        fun getAllCachedProjects(projectsDirectory: File): List<CachedProject> {
            ...
            return projectsDirectory.listFiles { file -> file.isDirectory }
                ...
                ?.sortedBy { it.info.projectTreeUri }
                ?: emptyList()
        }
    }
}
